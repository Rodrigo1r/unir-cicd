...
test-unit:
	docker run --name unit-tests --env PYTHONPATH=/opt/calc -w /opt/calc calculator-app:latest pytest --cov --cov-report=xml:results/pruebas/unit_test/coverage.xml --cov-report=html:results/pruebas/unit_test/coverage --junit-xml=results/pruebas/unit_test/unit_result.xml -m unit || true
	docker cp unit-tests:/opt/calc/results/pruebas/unit_test ./
